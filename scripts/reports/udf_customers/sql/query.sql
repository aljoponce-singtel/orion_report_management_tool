...
FROM
    ...
    LEFT JOIN RestInterface_customer CUS ON CUS.id = BRN.customer_id
    LEFT JOIN RestInterface_circuit CKT ON CKT.id = ORD.circuit_id
WHERE
    BRN.brn IN (
        '199405882Z',
        '197600379E',
    ...
    AND DATE(ORD.created_at) BETWEEN DATE_SUB(DATE(NOW()), INTERVAL 1 DAY)
    AND DATE(NOW())
ORDER BY
    'DateAddedToOrion',
    'CustomerName',
