...
def os_zip(csvfile,zipfile):
    os.system("zip -e -j %s %s -P Wq32Vfgg" % (zipfile, csvfile))
    os.remove(csvfile)
        ...
        FROM
            ...
            JOIN
                ...
                ON
                    ACT.order_id = ORD.id
        WHERE
            ORD.current_crd <= DATE_ADD(NOW(), INTERVAL 3 MONTH)
            AND ORD.order_status in ('Submitted', 'PONR', 'Pending Cancellation')
...
def war_query_step2():
    query_detail = """
        SELECT
          ...
          , CIR.circuit_code AS CircuitTie
          , CASE
                WHEN T_ED_PD.parameter_name IS NOT NULL
                    THEN T_ED_PD.parameter_value
                    ELSE 'NA'
            END AS Diversity
        FROM
            ...
            LEFT JOIN
                RestInterface_site SIT
                ON
                    ORD.site_id = SIT.id
            LEFT JOIN
                RestInterface_circuit CIR
                ON
                    ORD.circuit_id = CIR.id
            LEFT JOIN
                (
                    SELECT DISTINCT
                        order_id
                ...
                T_ED_PD
                ON
                    ORD.ID=T_ED_PD.order_id
            LEFT JOIN
                RestInterface_person PER
                ON
                    PER.id = ORD.person_id
            LEFT JOIN
                EscalationMatrix MTX
                ON
                    MTX.GroupID = PER.role
        ;
        """
    return query_detail
...
Orion Team
    ...
    emailBodyhtml = """\
        <html>
        <p>Hi All,</p>
        <p>Please have the attached ORION report excluding CFU 40, 41, 43.</p>
        <p>&nbsp;</p>
    ...
    subject = "[GSP War Room Report] {}{}{} {}{}".format(
        year, month, day, hour, ampm)
    sender = "orion@ncs.com.sg"
    receiver = ''
    receiverTo = 'petchiok@singtel.com;harishchandra.tilkar@singtel.com;christian.lim@singtel.com;ml-ttpctl@singtel.com;joyceline@singtel.com;hafizah@singtel.com'
    ...
    #receiverCc = 'maisarahm@singtel.com;jiangxu.jiang@singtel.com'
