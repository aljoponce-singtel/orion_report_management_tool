    ...
    (
        CASE
            WHEN ORD.service_number REGEXP "^M[0-9]{7}$"
                AND PRD.network_product_code REGEXP "^DME[0-9]{4}$"
                THEN "MetroE"
            WHEN ORD.service_number REGEXP "^M[0-9]{7}$"
                AND PRD.network_product_code REGEXP "^ISD[0-9]{4}$"
                THEN "ISDN30"
            WHEN ORD.service_number REGEXP "^GW[0-9]{7}$"
                OR ORD.service_number REGEXP "^GWM[0-9]{7}$"
                THEN "Gigawave"
            WHEN PRD.network_product_code REGEXP "^CPE[0-9]{4}$"
                THEN "MS-CPE"
            WHEN ORD.service_number REGEXP "^RMS00000[0-9]{5}$"
                THEN "MS - RMS"
            WHEN ORD.service_number REGEXP "^001[0-9]{5}SNG$"
                THEN "Cplus IPVPN ( in SG) (GSP)"
            WHEN ORD.service_number REGEXP "^001[0-9]{5}(?!SNG)[A-Z]{3}$"
                OR ORD.service_number REGEXP "^[A-Z]{3}[0-9]{10}LLC$"
                THEN "Cplus IPVPN (Overseas) (GSP)"
        END
    )
    AS product_name
  ,
    (
        CASE
            ...
            WHEN ORD.service_number REGEXP "^GW[0-9]{7}$"
                THEN "Gigawave"
            WHEN ORD.service_number REGEXP "^GWM[0-9]{7}$"
                THEN "Gigawave Monitoring"
            WHEN PRD.network_product_code REGEXP "^CPE[0-9]{4}$"
                THEN "CPE"
            WHEN ORD.service_number REGEXP "^RMS00000[0-9]{5}$"
                THEN "RMS"
            ...
            WHEN ORD.service_number REGEXP "^001[0-9]{5}(?!SNG)[A-Z]{3}$"
                THEN "Cplus PE Port"
            WHEN ORD.service_number REGEXP "^[A-Z]{3}[0-9]{10}LLC$"
                THEN "OLLC (resale data service)"
        END
    )
                             AS exp_type_of_work
