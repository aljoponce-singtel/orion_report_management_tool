...
def generateSDOReport(zipFileName, startDate, endDate, emailSubject):
    ...
    sqlquery = """
                ...
                FROM
                    t_GSP_ip_svcs
                WHERE
                    report_id IN ('GSDT7', 'GSDT8')
                    AND DATE(update_time) = '2022-05-01'
                ORDER BY
                    report_id;
                """

    csvFile = ("gsp_sdo_{}.csv").format(utils.getCurrentDateTime())
    outputList = tableauDb.queryToList(sqlquery)
