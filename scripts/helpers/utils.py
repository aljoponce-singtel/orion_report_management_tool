...
class Utils:
    ...
    def sort_df_by_priority_sequence(self, df: pd.DataFrame, column_name: str, priority_seq_list, keep_top_record=True) -> pd.DataFrame:
    ...
    def write_to_csv(self, csv_file_path, dataset, headers, folder_path):
    ...
    def compress_files(self, files_to_zip, zip_file_path, password=None, remove_file=True):
        ...
        if platform == "Linux":
            self.os_zip_file(files, zip_file_path, password, remove_file)
        elif platform == "Windows":
            self.zip_file(files, zip_file_path, remove_file)
        else:
            raise Exception(f"OS {platform} not supported.")

    def zip_file(self, files_to_zip, zip_file, remove_file=True):
    ...
    def os_zip_file(self, files_to_zip, zip_file, password=None, remove_file=True):
    ...
    def get_prev_week_monday_sunday_date(self, date):
        # Example:
        ...
        date_obj = self.to_date_obj(date)
        ...
        last_week_monday = date_obj - \
            timedelta(days=days_since_last_monday + 7)
        # Calculate the date of the last week's Sunday
        last_week_sunday = date_obj - \
            timedelta(days=days_since_last_monday + 1)

        return last_week_monday, last_week_sunday

    def get_prev_month_first_last_day_date(self, date):
        # Example:
        ...
        # start date
        start_date = self.subtract_months(date_obj, 1).replace(day=1)

        # end date
        prev_month_date = self.subtract_months(date_obj, 1)
        end_date = self.get_last_day_of_month(prev_month_date)

        return start_date, end_date

    def subtract_months(self, date, no_of_months):
    ...
    def get_last_day_of_month(self, date_input=None):
        # Example:
    ...
    def create_folder(self, folder_path, overwrite=False):
    ...
    def scan_files(self, dir_path, regex=None):
    ...
    def move_file(self, source_path, destination_path):
        ...
        logger.info(
            f"Moving {basename(source_path)} to {destination_path} ... ")
    ...
    def add_ext_type(self, filename, new_extension):
        new_filename = f"{filename}.{new_extension}"  # Create the new filename
        return new_filename

    def replace_ext_type(self, filename, new_extension):
    ...
    def set_excel_password_linux(self, file_path, password):
        logger.info('Setting excel password ...')

        try:
            # Open the original Excel file
            workbook = load_workbook(self, filename=file_path)
    ...
    def set_excel_password_windows(self, file_path, password):
        logger.info('Setting excel password ...')
    ...
    def set_excel_password(self, file_path, password, replace=True, append_string='_protected'):

        if replace == False:
            protected_file_path = file_path.split(
                '.xlsx')[0] + ('{}.xlsx').format(append_string)
            self.copy_file(file_path, protected_file_path)
            file_path = protected_file_path

        platform = self.get_platform()

        if platform == 'Linux':
            self.set_excel_password_linux(file_path, password)
        elif platform == 'Windows':
            self.set_excel_password_windows(file_path, password)
        else:
            raise Exception(f"OS {platform} not supported.")

    # Only supports Windows for now

    def open_file_using_default_program(self, file_path, app_name=None):
    ...
    def get_ppid_of_process(self, target_pid):
    ...
    def was_called_by_cronjob(self, pid=None):
        ...
        if self.get_platform() == 'Linux':
            try:
                ...
                if pid:
                    ppid = self.get_ppid_of_process(pid)
                else:
                    ppid = os.getppid()
                ...
                if ppid:
                    ...
                    with open(f'/proc/{ppid}/comm', 'r') as f:
                        ...
                        logger.debug(
                            f"parent_process_name: {parent_process_name}")
